...
var SUBCATEGORY_MANAGER_ITEM = $('.subcategory-manager_item');
...
$('body').delegate('button[name="subcategory-manager_delete"]','click',function(){
  ...
  $('#wallpaper-admin-preloader').css({'display':'flex'});
  $.post('../wallpaper/php/select.php',{data:sql},function (data){
    $('#wallpaper-admin-preloader').css({'display':'none'});
    var arr=JSON.parse(data);
    if (arr.length>0) {
      alert('Объект содержит вложенные элементы. Удаление невозможно!');
    }else {
...
$('.subcategory-manager_add').click(function () {
  ...
  } else {alert('Укажите название подкатегории!');}
});
  ...
  if (image.files.length<1) {
    ...
    request.onreadystatechange=function (){
      if (request.readyState==4 && request.status==200){
        ...
        $('.file-upload_info').html('');
        $('select[name="wallpaperInterior"]').children('option:first').prop('selected', true);
        $('select[name="wallpaperCategory"]').children('option:first').prop('selected', true);
        $('select[name="wallpaperCategory"]').trigger('change');
        var inputs_arr = $('#wallpaper-upload-popup').find('input');
...
function subcategoryManagerForm (arr) {
  var tr=SUBCATEGORY_MANAGER_ITEM;
  $('.subcategory-manager_item').remove();
  for (var i = 1; i <= arr.length; i++) {
