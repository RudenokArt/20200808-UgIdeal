...
$('button[name="wallpaper_constructor-favorite_add"]').click(function () {
  ...
  if (cartItemCompare(item,arr)) {arr.push(item);}
...
$('body').delegate('input[name="texture_radio"]','change',function () {
...
$('body').delegate('.wallpaper_texture-block_item i','click',function () {
  $('#popup_'+this.id).css({
    'display':'flex',
  });
});
$('body').delegate('button[name="video_button"]', 'click', function () {
  $('.wallpaper_constructor-video_block').fadeIn();
});
$('body').delegate('button[name="video_button-close"]','click',function () {
 stopVideo();
 $('.wallpaper_constructor-video_block').fadeOut();
});
$('body').delegate('button[name="wallpaper_constructor-popup_close"]', 'click', function () {
  stopVideo();
...
$('#wallpaper_texture-block_arrow-left').click(texture_scroll_left);  

$('#wallpaper_texture-block_arrow-right').click(texture_scroll_right);



// ========= FUNCTIONS =========

function texture_scroll_right () {
  var block = '#wallpaper_texture-block_tape';
  var arr = $('#wallpaper_texture-block label');
  $(block).html('');
  var res_arr = [];
  res_arr[arr.length-1] = arr[0];
  for (var i = 0; i < arr.length-1; i++) {
    res_arr[i] = arr[i+1];
  }
  $(block).html(res_arr);
  $(block).css({'margin-left':'0px','margin-right':'-200px'});
  $(block).animate({
    'margin-left':'-=100px',
    'margin-right':'+=100px',
  },500,texture_slider());
...
function texture_scroll_left () {
  var block = '#wallpaper_texture-block_tape';
  var arr = $('#wallpaper_texture-block label');
  $(block).html('');
  var res_arr = [];
  res_arr[0] = arr[arr.length-1];
  for (var i = 0; i < arr.length-1; i++) {
    res_arr[i+1] = arr[i];
  }
  $(block).html(res_arr);
  $(block).css({'margin-left':'-200px','margin-right':'0px'});
  $(block).animate({
    'margin-left':'+=100px',
    'margin-right':'-=100px',
  },500,texture_slider());
...
function cartItemCompare(item,arr) {
  var flag = true;
  item = item.split('|')[0];
  arr.forEach( function(element, index) {
    if (arr[index].split('|')[0]==item) {
      flag = false;
    }
  });
  return flag;
...
function orderData () {
  var imageId = localStorage.getItem('wallpaper_constructor');
  imageId = imageId.split('|')[0];
  var arr = [];
  var str = localStorage.getItem('wallpaper_cart');
  if (str == null) {
    arr = [];
  } else {
    arr = JSON.parse(str);
  ...
  arr.forEach( function(element, index) {
    if (element[0] == imageId){
      arr.splice(index, 1);
    }
  });
  arr.push([imageId,wallpaper]);
  var json = JSON.stringify(arr);
  localStorage.setItem('wallpaper_cart', json);
}
...
function texture_slider () {
  var arr = $('#wallpaper_texture-block label');
  $(arr).hide();
  var quantity = texture_slider_quantity();
  for (var i = 0; i < quantity; i++) {
    $(arr[i]).show();
  }
}
function texture_slider_quantity () {
  var width = $('#wallpaper_texture-block_wrapper')[0].offsetWidth;
