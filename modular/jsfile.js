...
function editItem(){
	...
	var str=JSON.stringify(itemEdit)
	ajaxEditItem(str);
}
...
function ajaxGallery(where) {
	if (where) {
		localStorage.setItem('modularGalleryFilter', 'Y');
	} else {
		localStorage.setItem('modularGalleryFilter', 'N');
	}
	var req = new XMLHttpRequest();
...
function displayGalery_options (item) {
	var image_options ='';
	if (
		item.image_size != 0
		|| item.horizontal_position != 0
		|| item.vertical_position != 0
		|| item.scale_x != 0
		|| item.scale_y != 0
		|| item.image_rotate != 'rotate(0deg)'
		) 
	{
		var scale_x, scale_y;
		if (item.scale_x == 0) {
		...
		'transform: scale('+scale_x+', '+scale_y+') '+item.image_rotate+';'+
		'background-position: '
		+item.horizontal_position+'% '+item.vertical_position+'%;';
	}
...
{	
	try {
		...
		for (i=0;i<arr.length;i++) {
			var image_options = displayGalery_options(arr[i]);
			...
			var galeryName=(arr[i].galeryName);
			if (discount==0)
				{discount='';}
      ...
      '<div class="galery-image-wrapper">'+
			'<div class="galery-image" style="'+ image_options +
			'background-image: url(galery/'+arr[i].image+');">'+
			'<div class="discount">'+discount+'</div>'+
			'</div>'+
      '<div style="background-image: url(mini-templates/'+template+');" class="admin_galary_template"></div>'+
      '</div>'+
			'<div>'+
			...
			'</div>'+
			'<div>'+
			'<button class="gallery_image_position btn btn-outline-light"'+
			'onclick="event.stopPropagation()" name="gallery_image_position.php?template='+template+'&image_id='+id+'">'+
		...
		$('.gallery_image_position').click(imagePositionPopUp);
		pagination();
	} catch(e) {}
}


function imagePositionPopUp () {
	$('#imagePositionPopUp-closeButton').click(function () {
		$('#imagePositionPopUp-wrapper').css({'display':'none'});
	});
	$('#imagePositionPopUp-wrapper').css({'display':'flex'});
	$('#imagePositionPopUp-preloader_wrapper').css({'display':'flex'});
	var url =  this.name.replace(' ', '%20');
	$('#imagePositionPopUp-inner').load(url, function () {
...
>>>>>>> a214cf798aeb10c6ebad9dc9e55f17466cf591cb
